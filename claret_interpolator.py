""" Define two classes inherited from 
general_purpose_python_modules.cmd_utils.cmd_isochrone_interpolator. 
This file should be saved in the same folder as 
general_purpose_python_modules.cmd_utils.cmd_isochrone_interpolator."""
from general_purpose_python_modules.cmd_utils.modified_cmd_isochrone_interpolator import (
    IsochroneFileIterator as iterator,
)
from general_purpose_python_modules.cmd_utils.modified_cmd_isochrone_interpolator import (
    CMDInterpolator as interpolator,
)
import numpy
...
class ClaretInterpolator(interpolator):
    """
    An new class inherit the CMDInterpolator with new initial function and 
    _get_interpolation_grid
    """
...
class ClaretFileIterator(iterator):
    """
    Claret File Iterator Just like the IsochroneFileIterator in original 
    general_purpose_python_modules with different
    strings to be checked by the function. Since the data file from claret data 
    has different header and columns.
    """
    ...
    def __enter__(self):
        ...
        while True:
            ...
            assert (
                self._line[0] == "#"
                or self._line.startswith("---")
                or self._line.startswith("yr|")
                or self._line.startswith("Age")
            )
